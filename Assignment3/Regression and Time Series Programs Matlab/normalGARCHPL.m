...
e=(abs(epsi)).^delta;  lambda=((2^(delta/2))/sqrt(pi))*gamma((delta+1)/2);
% sinit =  E[sigvec_0^delta], einit = E|e_0^delta| = lambda * sinit.
sinit=mean((abs(epsi)).^delta); einit=lambda*sinit;
sigvec=zeros(length(e),1); % do the recursion in sigvec^delta
sigvec(1)=int+qterms*einit+pterms*sinit;
